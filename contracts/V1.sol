...
contract KIP17Metadata is KIP13, KIP17, IKIP17Metadata {
    ...
    constructor (string memory name, string memory symbol) public {
        ...
        _baseUri = "https://api.cipher-assembly.com/nfts/ciph/";
    }
    ...
    /**
     * @dev Returns an URI .addressfor a given token ID.
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        //uint2str(tokenId)
        return string(abi.encodePacked(base, integerToString(tokenId)));
    }
...
contract KIP17Token is KIP17Full, KIP17Burnable, KIP17Pausable {
...
contract V1 is KIP17Token("CipherAssembly", "ciph"), Ownable {
    using SafeMath for uint256;

    uint256 public nextMintIndex = 1;
    uint256 public maxSupply = 5000;
    uint256 public mintPrice = 10000000000000000000;
    ...
    function mintBatch(address newOwner, uint amount) external onlyOwner {
        require(nextMintIndex.add(amount)<maxSupply+2, "Max supply exceeded");
    ...
    function mintOne(address newOwner) external onlyOwner {
        require(nextMintIndex<maxSupply+1, "Max supply exceeded");
        _mint(newOwner, nextMintIndex);
        nextMintIndex++;
    }

    function mintPublic(address newOwner, uint256 amount) external payable {
        require(nextMintIndex.add(amount) < maxSupply+2, "Purchase would exceed max supply");
        require(mintPrice.mul(amount) <= msg.value, "Klay value sent is not correct");

        for(uint256 i = 0; i < amount; i++) {
            if (nextMintIndex<maxSupply+1) {
                _mint(newOwner, nextMintIndex);
                nextMintIndex++;
            }
        }
